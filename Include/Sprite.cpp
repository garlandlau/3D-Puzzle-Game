...
void Sprite::SetScreenPosition(int x, int y) {
	ViewportSize(winWidth, winHeight);
	vec2 ndc((2.f*x)/winWidth-1, (2.f*y)/winHeight-1);
	SetPosition(ndc);
}

vec2 Sprite::GetScreenPosition() {
	ViewportSize(winWidth, winHeight);
	float x = (position.x+1)*winWidth/2.f;
	float y = (position.y+1)*winHeight/2.f;
	return vec2(x, y);
}
...
void Sprite::UpdateTransform() {
	...
	if (compensateAspectRatio) {
		int vp[4]; // why not use winWidth, winHeight?
		glGetIntegerv(GL_VIEWPORT, vp);
		float w = (float) vp[2], h = (float) vp[3], aspectRatio = w/h;
		w > h? s.x /= aspectRatio : s.y *= aspectRatio;
	}
	ptTransform = Translate(position.x, position.y, 0)*Scale(s)*RotateZ(rotation);
}
