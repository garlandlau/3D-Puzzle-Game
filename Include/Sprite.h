...
public:
	...
	// geometry
	bool compensateAspectRatio = false;	// if true, adjust scale given window size
	...
	vec2 Drag(double x, double y);				// move
	...
	Sprite(vec2 p = vec2(), float s = 1, bool invVrt = true) : position(p), scale(vec2(s, s)) {  }
	Sprite(vec2 p, vec2 s, bool invVrt = true) : position(p), scale(s) { }
	~Sprite() { Release(); }
};
